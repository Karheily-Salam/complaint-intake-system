...
asks the customer to clarify rather than guessing — a wrong type would collect
the wrong fields and produce a useless ticket. Classification is re-attempted
...
volunteers are absorbed rather than asked for again, so answering three
...
so it advertises its own token rather than the previous thread's — without
...
the engine actually changed a collected field** that turn. Restating a value or
saying "thanks" changes nothing and warrants no email. Support is notified
exactly once, when the ticket is created.
...
it holds however the match was made rather than depending on each call site
...
- **List** — grouped by complaint type (deposits, withdrawals, other), with
  ...
  true totals rather than whatever landed on the current page, and the browser
...
SQLite runs with WAL (a write does not block readers — this process has both an
API and a poller writing), a 30s busy timeout, and `foreign_keys=ON`, which
SQLite otherwise ignores. Backups are timestamped, gzipped and taken with
SQLite's online-backup API, never a raw file copy of a live database.
...
- **Prompt injection cannot alter workflow state** — see the design rule.
...
294 tests, run in CI on every push and pull request alongside the frontend
...
Coverage is behavioural rather than incidental — the conversation lifecycle
...
and the AI-versus-deterministic boundary.
