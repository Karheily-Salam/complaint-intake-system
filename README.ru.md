...
| **Развёрнуто** (http://startplus.tech/) | `mock` — ящик не подключён | только синтетические демонстрационные записи |
| **Настоящая почта** (`imap_smtp`) | реализовано и покрыто тестами, но не включено | нужен ящик и учётные данные |

Интеграция с IMAP/SMTP завершена и покрыта тестами; развёртывание просто ещё
не направлено на живой почтовый ящик. Что осталось — в разделе
...
flowchart TD
    ...
    P["EmailPoller<br/>(IMAP, каждые 60 с)"]
...
backend/
  app/
    ...
    ai/               абстракция AIProvider + rule-based и Ollama провайдеры
  ...
  scripts/            check_email.py (проверка ящика), seed_demo.py
  tests/              294 теста
...
```bash
cd backend
../.venv/Scripts/python.exe -m pytest      # 294 теста
...
- **Опрос, а не push.** Интервала около 60 секунд достаточно для приёма
  обращений, и он не требует публичной точки для вебхуков. Доставка за доли
  секунды означала бы IMAP IDLE или провайдера с вебхуками — это другой
  компромисс, а не улучшение.
- **Сегодня в продакшене mock-почта.** Интеграция с IMAP/SMTP завершена и
  покрыта тестами, но пока не подключена к живому ящику.
- **HTTPS пока нет** — домен на хост не направлен. До тех пор демонстрация
  работает по обычному HTTP, и это ещё одна причина, почему на публичной
  поверхности нет настоящих данных.
